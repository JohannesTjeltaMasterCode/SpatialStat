...
if __name__=='__main__':
    InputName=input('Enter energy of protons:')
    ...
    if Method==1:
        parameter_limit=int(input('Input cut-value [nm]:'))
    if Method==0:
    ...
    for lol,i in enumerate(N_array):
        ...
        if Method==2:
            ...
            #Doselist=['1Gy','2Gy','3Gy','Gy','8Gy','10Gy']
            if InputPath=='Data1_2':
                EnergyName='1.2MeV Protons'
                savename='1_2mevprotons'
            elif InputPath=='Data1_4':
                EnergyName='1.5MeV Protons'
                savename='1_5mevprotons'
            elif InputPath=='Data1_8':
                EnergyName='1.8MeV Protons'
                savename='1_8mevprotons'
            elif InputPath=='Data8':
                EnergyName='8.7MeV Protons'
                savename='8_7mevprotons'
            nuc_h=4
            def plot_circle():
                n=2000
                whole_pi=np.linspace(0,2*np.pi,n)
                c1=np.zeros(n)
                c2=np.cos(whole_pi)*6*1
                c3=np.sin(whole_pi)*6*1
                return c1,c2,c3
            qwe=plot_circle()
            ax = plt.axes(projection='3d')
            for i in range(np.shape(ion_tot)[0]):
                ax.plot3D(ion_tot[i,0,:]/1000,ion_tot[i,1,:]/1000,ion_tot[i,2,:]/1000,color='tomato',
                            marker='.',ms=450./300, mew=0, linestyle="", lw=0)
                if i==0:

                    ax.plot3D(qwe[1],qwe[2],qwe[0],'--',color='gray')
                    ax.plot3D(qwe[1],qwe[2],np.ones(2000)*nuc_h,'--',color='gray')
                    ax.legend(['Proton and electron events','Nucleus border'])
            ...
            ax.set_xlabel('Dist [$\mu m$]')
            ax.set_ylabel('Dist [$\mu m$]')
            ax.set_zlabel('Dist [$\mu m$]')

            #plt.title('Nucleus irradiated with {}, {}'.format(EnergyName,Doselist[lol]))
            plt.savefig('../Plots/CellDosePlot/Protontrack{}_{}.jpeg'.format(str(N),savename),bbox_inches='tight',dpi=300)
            #plt.show()
    #ax1.set(ylabel='Dist [$\mu m$]')
    #fig.xlabel('Dist [$\mu m$]')
    #fig.ylabel('Dist [$\mu m$]')
